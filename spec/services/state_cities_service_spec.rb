...
RSpec.describe StateCitiesService, type: :service do
  describe '.states' do
    it 'returns an array of state codes' do
      allow(HttpConnection::Adapter).to receive(:get).and_return(double(status: 200, body: '[{"sigla": "RN"}]'))

      expect(StateCitiesService.states).to eq(['RN'])
    end

    it 'returns default state code if API response is not successful' do
      allow(HttpConnection::Adapter).to receive(:get).and_return(double(status: 500))
  ...
  describe '.cities' do
    it 'returns an array of city names for the given state code' do
      allow(HttpConnection::Adapter).to receive(:get).and_return(double(status: 200, body: '[{"nome": "Natal"}]'))
    ...
    it 'returns an empty array if the state code is not provided' do
      allow(HttpConnection::Adapter).to receive(:get).and_return(double(status: 200, body: '[]'))
